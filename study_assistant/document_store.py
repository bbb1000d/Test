...
class DocumentStore:
    ...
    def is_trusted(self, document_or_source: Union[Document, str]) -> bool:
        """Return ``True`` when a document stems from a trusted source."""

        source = document_or_source.source if isinstance(document_or_source, Document) else document_or_source
        return self._is_trusted_source(source)
