...
def run() -> None:
    ...
    parser.add_argument("--no-color", action="store_true", help="Disable ANSI colors in the console output")
    parser.add_argument("--width", type=int, default=None, help="Override console width for formatted output")
    ...
    mode_titles = {
        ...
        "weaknesses": "Weakness Diagnosis",
        "micro": "Micro Assessment",
        "capstone": "Capstone Assessment",
        "apply": "Applied Scenario",
        "plan": "Adaptive Study Plan",
    }

    extras = []
    if args.mode == "micro":
        extras = [("Quiz scope", args.quiz_scope), ("Question count", str(args.quiz_questions))]
    elif args.mode == "capstone":
        extras = [("Assessment weight", args.capstone_weight), ("Style", args.capstone_style)]
    elif args.mode == "apply":
        extras = [("Scenario", args.scenario)]
    ...
    subtitle = f"{args.topic} • {mode_titles.get(args.mode, args.mode.title())}"
    console = StyledConsole(width=args.width, enable_color=not args.no_color)
    console.print_header("Study Assistant", subtitle=subtitle)
    console.print_overview(topic=args.topic, focus=args.focus, mode_label=mode_titles[args.mode], extras=extras)
    console.print_documents(store.list_documents(), store=store)
    console.print_profile(profile)
    console.print_result(mode_titles[args.mode], output)
